...
import { PaymentsQuickstartSetup } from './payments/quickstart/setup';
...
export class Setup extends SfdxCommand {
    ...
    public async run(): Promise<AnyJson> {
        ...
        this.ux.log(chalk.green('Authing up d evhub'));
        ...
        for (scratchOrg; scratchOrg < scratchOrgTotal; scratchOrg++) {
            ...
            if (!output)
                throw new SfdxError(
                    messages.getMessage('setup.errorScratchOrgCreate', [devHubConfig.scratchOrgAdminUsername])
                );
