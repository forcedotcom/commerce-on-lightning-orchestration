        ...
        try {
            await page.evaluate(() => {
                Array.from(document.querySelectorAll('label'))
                    .filter(
                        (e) =>
                            ['B2CLiteAccess', 'CommerceEnabled'].indexOf(e.innerText) >= 0 &&
        ...
        try {
            ...
            if (e.message.indexOf('MyDomainResolverTimeoutError') >= 0) {
                ...
                if (message.name === 'DevhubNotAuthorized' || message.name === 'genericTimeoutMessage') {
                    this.log(
                        chalk.red(
                            msgs.getMessage('create.errorOccurredMightNeedDevhubAuth') + (message.message as string)
                        )
                    );
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
                    ...
                    throw new SfdxError(
                        msgs.getMessage('create.errorOccurredDuringOrgCreateMightNeedTMPAuth') +
